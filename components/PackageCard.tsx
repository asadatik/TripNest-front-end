  ...
  return (
    ...
    >
      <Card className="group relative h-full overflow-hidden border-2 border-slate-200 bg-white shadow-lg transition-all duration-300 hover:border-cyan-500 hover:shadow-2xl hover:shadow-cyan-500/20 dark:border-slate-800 dark:bg-slate-900 dark:hover:border-cyan-400 dark:hover:shadow-cyan-400/20">
        ...
        {/* Image */}
        {pkg.images?.length > 0 && (
          <div className="group relative aspect-video overflow-hidden bg-gradient-to-br from-slate-200 to-slate-300 dark:from-slate-800 dark:to-slate-900">
        ...
        <CardContent className="flex flex-1 flex-col justify-between space-y-4 pt-0">
          ...
          <Link href={`/packages/${pkg.slug}`} className="mt-auto">
            <motion.div whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }}>
              <Button className="group relative w-full overflow-hidden rounded-xl bg-gradient-to-r from-cyan-500 via-blue-600 to-purple-500 py-6 text-base font-semibold text-white shadow-lg shadow-cyan-500/30 transition-all hover:shadow-xl hover:shadow-cyan-500/40">
                <span className="relative z-10 flex items-center justify-center gap-2">
                  View Details
                  <ArrowRight className="h-5 w-5 transition-transform group-hover:translate-x-1" />
                </span>
                <motion.div
                  className="absolute inset-0 -z-10 bg-gradient-to-r from-cyan-600 via-blue-700 to-purple-500"
                  initial={{ x: "-100%" }}
                  whileHover={{ x: "0%" }}
                  transition={{ duration: 0.3 }}
                />
