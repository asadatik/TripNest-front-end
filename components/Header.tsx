...
export default function Header() {
  ...
  // Detect if scrolled for border animation
  useEffect(() => {
    const handleScroll = () => {
  ...
  return (
    <>
      {/* Scroll progress bar */}
      <motion.div
        className="fixed top-20 left-0 h-[2px] z-50"
        style={{
                ...
                <Link key={item.href} href={item.href}>
                  <motion.div
                    className={`relative rounded-full px-5 py-2.5 text-sm font-semibold transition-all ${
                      active
                        ? "text-transparent bg-gradient-to-r from-cyan-600 to-blue-600 bg-clip-text dark:from-cyan-400 dark:to-blue-500"
                      ...
                      <DropdownMenuItem key={item.href} asChild className="cursor-pointer">
                        <Link
                          ...
                          className={`flex items-center rounded-lg px-3 py-2.5 text-sm font-medium transition-colors ${
                            active
                              ? "bg-gradient-to-r from-cyan-100 to-blue-100 text-cyan-700 dark:from-cyan-950/50 dark:to-blue-950/50 dark:text-cyan-300"
                              : "text-slate-700 transition-colors hover:bg-slate-100 dark:text-slate-200 dark:hover:bg-slate-800"
                          }`}
                        >
                          {item.label}
                        </Link>
